        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (args[0].EndsWith(".acb", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    using (CriTableReader acbReader = CriTableReader.Create(args[0]))
                    {
                            ...
                            if (afs2Archive.SubKey != 0)
                            {
                                using (
                                    var stream = File.Create(
                                        Path.Combine(outputDirectoryPath, ".subkey")
                                    )
                                )
                        ...
                        if (acbReader.GetLength("StreamAwbAfs2Header") > 0)
                        {
                            ...
                            using (
                                SubStream extAfs2Stream = acbReader.GetSubStream(
                                    "StreamAwbAfs2Header"
                                )
                            )
                            {
                                ...
                                {
                                    using (
                                        CriTableReader utfAfs2HeaderReader = CriTableReader.Create(
                                            extAfs2Stream
                                        )
                                    )
                                    {
                                        ...
                                        using (
                                            SubStream extAfs2HeaderStream =
                                                utfAfs2HeaderReader.GetSubStream("Header")
                                        )
                                        {
                            ...
                            if (!found)
                            {
                                throw new FileNotFoundException(
                                    "Unable to locate the corresponding streaming AWB file. Please ensure that it's in the same directory."
                                );
                            }
                            ...
                            if (extAfs2Archive.SubKey != 0)
                            {
                                using (
                                    var stream = File.Create(
                                        Path.Combine(outputDirectoryPath, ".subkey_streaming")
                                    )
                                )
                        ...
                        using (
                            SubStream waveformTableStream = acbReader.GetSubStream("WaveformTable")
                        )
                        using (
                            CriTableReader waveformReader = CriTableReader.Create(
                                waveformTableStream
                            )
                        )
                        {
                            while (waveformReader.Read())
                            {
                                ...
                                ushort id = waveformReader.ContainsField("MemoryAwbId")
                                    ? streaming
                                        ? waveformReader.GetUInt16("StreamAwbId")
                                        : waveformReader.GetUInt16("MemoryAwbId")
                ...
                else if (File.GetAttributes(args[0]).HasFlag(FileAttributes.Directory))
                {
                    ...
                    {
                        throw new FileNotFoundException(
                            "Unable to locate the corresponding ACB file. Please ensure that it's in the same directory."
                        );
                    }
                    ...
                    cpkMode =
                        !(
                            awbFile != null
                            && awbFile.Length >= 4
                            && Encoding.ASCII.GetString(awbFile, 0, 4) == "AFS2"
                        ) && (streamAwbAfs2Header == null || streamAwbAfs2Header.Length == 0);

                    using (
                        CriTableReader reader = CriTableReader.Create(
                            (byte[])acbFile.Rows[0]["WaveformTable"]
                        )
                    )
                    {
                        while (reader.Read())
                        {
                            ...
                            ushort id = reader.ContainsField("MemoryAwbId")
                                ? streaming
                                    ? reader.GetUInt16("StreamAwbId")
                                    : reader.GetUInt16("MemoryAwbId")
                                : reader.GetUInt16("Id");
                    ...
                    if (afs2Archive.Count > 0 || cpkArchive.Count > 0)
                    {
                        ...
                        acbFile.Rows[0]["AwbFile"] = cpkMode
                            ? cpkArchive.Save()
                            : afs2Archive.Save();
                        Console.WriteLine();
                    }
